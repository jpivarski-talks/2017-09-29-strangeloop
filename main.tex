...
\vspace{0.75 cm}
\uncover<2->{For the new database-style to work,}

\vspace{0.25 cm}
\begin{minipage}{0.8\linewidth}
\begin{itemize}\setlength{\itemsep}{0.25 cm}
\item<3-> \Large responses must be rapid enough for end-user analysis (seconds per plot)
\item<4-> \Large the interface must allow for algorithms on nested objects.
\end{itemize}
\end{minipage}
\end{frame}
...
\normalsize
\vspace{0.2 cm}
\uncover<2->{Other common types (such as strings) can be constructed from these (arbitrary-length list of characters, for instance).}

\vspace{0.2 cm}
\uncover<3->{It's easier to implement code transformation (AST node rewriting) with as few complications as possible.}

\vspace{0.2 cm}
\uncover<4->{\mbox{ } \hfill \textcolor{blue}{\url{https://github.com/diana-hep/plur}} \hfill \mbox{ }}
\end{frame}
...
\vspace{0.75 cm}
\uncover<2->{One important difference: PLUR does not require columns to be laid out in a contiguous block. They may be memory mapped files, generated on demand, streaming from the network, etc.}
\end{center}
\end{columns}
\end{frame}
...
\begin{uncoverenv}<2->
\begin{center}
\only<1-2>{\includegraphics[width=0.8\linewidth]{augmentation-example-1.pdf}}
\only<3>{\includegraphics[width=0.8\linewidth]{augmentation-example-2.pdf}}
\only<4>{\includegraphics[width=0.8\linewidth]{augmentation-example-3.pdf}}
\only<5>{\includegraphics[width=0.8\linewidth]{augmentation-example-4.pdf}}
\end{center}
\end{uncoverenv}
\end{frame}
