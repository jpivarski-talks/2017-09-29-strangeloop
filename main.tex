...
\item<4-> Repeating the process is so expensive that analysis groups hedge their bets by requesting events and attributes they're not sure they'll need.
...
\vspace{0.5 cm}
if we can provide a database-style server, the physicists would \\ have to use it for interactive analysis queries, not bulk downloads.
...
\vspace{0.5 cm}
Sure, we {\it store} them as columns (our ROOT format, similar to Parquet), but we also don't have to spend time deserializing them into objects.
...
\begin{uncoverenv}<3->
\mbox{\hspace{-0.1 cm}\only<3>{\underline{we should execute}}\only<4->{\underline{or even (special case of exhaustive nested loops)}}}
...
\vspace{0.5 cm}
\uncover<5->{\textcolor{darkblue}{The data representation is just Arrow; the code transformation can be automated.}}
\end{frame}
